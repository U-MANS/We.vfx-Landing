...
`;
    ...
    const color_frag_reveal = `
    precision highp float;
    uniform sampler2D velocity;
    uniform sampler2D palette;
    uniform vec4 bgColor;
    uniform sampler2D uRevealVideo;
    ...
    void main() {
      ...
      float lenv = clamp(length(vel), 0.0, 1.0);
      vec3 c = texture2D(palette, vec2(lenv, 0.5)).rgb;
      vec3 baseGrad = mix(vec3(0.014, 0.014, 0.02), vec3(0.048, 0.045, 0.072), pow(uv.y, 1.12));
      baseGrad += vec3(0.1, 0.03, 0.06) * (1.0 - uv.y) * (1.0 - uv.y) * 0.14;
...
`;
    ...
    class Output {
      constructor(revealVideoRef) {
        this.revealVideoRef = revealVideoRef;
        this.videoTexture = null;
        this._dummyRevealTex = null;
        this.init();
      }
      init() {
        ...
        this.scene = new THREE.Scene();
        this.camera = new THREE.Camera();
        const useReveal = Boolean(this.revealVideoRef);
        const uniforms = {
        ...
        let fragmentShader = color_frag;
        if (useReveal) {
          fragmentShader = color_frag_reveal;
          const px = new Uint8Array([0, 0, 0, 255]);
          this._dummyRevealTex = new THREE.DataTexture(px, 1, 1, THREE.RGBAFormat);
          this._dummyRevealTex.needsUpdate = true;
          uniforms.uRevealVideo = { value: this._dummyRevealTex };
          uniforms.uPointerUv = { value: new THREE.Vector2(0.5, 0.5) };
      ...
      ensureVideoTexture() {
        if (!this.revealVideoRef) return;
        const el = this.revealVideoRef.current;
        if (!el || this.videoTexture) return;
        if (el.readyState < 2) return;
        this.videoTexture = new THREE.VideoTexture(el);
        this.videoTexture.minFilter = THREE.LinearFilter;
        this.videoTexture.magFilter = THREE.LinearFilter;
        if (THREE.SRGBColorSpace) this.videoTexture.colorSpace = THREE.SRGBColorSpace;
        this.output.material.uniforms.uRevealVideo.value = this.videoTexture;
        if (this._dummyRevealTex) {
          this._dummyRevealTex.dispose();
          this._dummyRevealTex = null;
        }
      }
      disposeReveal() {
      ...
      updateRevealUniforms() {
        if (!this.revealVideoRef || !this.output.material.uniforms.uPointerUv) return;
        const m = Mouse.coords;
        this.output.material.uniforms.uPointerUv.value.set(m.x * 0.5 + 0.5, m.y * 0.5 + 0.5);
        this.output.material.uniforms.uAspect.value = Common.aspect;
        this.ensureVideoTexture();
        if (this.videoTexture) this.videoTexture.needsUpdate = true;
      }
      update() {
        this.simulation.update();
        this.updateRevealUniforms();
        this.render();
      }
    }
    ...
    class WebGLManager {
      constructor(props) {
        ...
        this._revealVideoRef = props.revealVideoRef ?? null;
        this.init();
      ...
      init() {
        ...
        this.output = new Output(this._revealVideoRef);
      }
      resize() {
        Common.resize();
        this.output.resize();
      ...
      dispose() {
        try {
          ...
          Mouse.dispose();
          if (this.output && typeof this.output.disposeReveal === 'function') {
            this.output.disposeReveal();
          }
          if (Common.renderer) {
